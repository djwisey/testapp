...
class PocketBaseService {
  PocketBaseService({PocketBase? client})
      : client = client ?? PocketBase('https://emnapi.dylanwiseman.com');
  ...
  Future<RecordModel> signIn(String email, String password) async {
    final auth = await client.collection('users').authWithPassword(
  ...
  Future<List<RecordModel>> getUsers() => client.collection('users').getFullList(
        sort: 'name',
      );
  ...
  Future<RecordModel> createWorker({
    ...
    return client.collection('users').create(body: <String, dynamic>{
      'name': name.trim(),
  ...
  Future<RecordModel> updateWorker({
    ...
    return client.collection('users').update(id, body: <String, dynamic>{
      'name': name.trim(),
      'email': email.trim().toLowerCase(),
      'active': active,
      'job_title': jobTitle.trim(),
  ...
  Future<void> resetWorkerPassword(String id, String password) async {
    await client.collection('users').update(id, body: <String, dynamic>{
      'password': password,
      'passwordConfirm': password,
    });
  }
  ...
  Future<List<RecordModel>> getJobs() => client.collection('jobs').getFullList(
        sort: '-scheduled_date,reference',
      );
